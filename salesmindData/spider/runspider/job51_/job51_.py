...
class JobAk(BaseJob):
    def __init__(self, param):
        super(JobAk, self).__init__(param=param)
    ...
    def parse_list(self):
        ...
        data = {
            ...
            'ctrlSerach$hidSearchValue': self.data,
            '__VIEWSTATE': viewstate
        }
        page = 1
        while True:
            if self.sign == 0:
                r = requests.post('https://ehire.51job.com/Candidate/SearchResumeNew.aspx', headers=self.headers, data=data,
                                  verify=False)
                html = etree.HTML(r.text)
    ...
    def run(self):
        try:
            ...
            if self.sign == 0:
                ...
                print('{}-url采集完成'.format(self.table_name))
            else:
                print('{}-url采集中断'.format(self.table_name))
        except Exception as E:
            print(str(E))
    ...
    def run(self):
        try:
            for data in self.get_data():
                if self.sign == 0:
                    item = self.parse_detail(data['data'])
